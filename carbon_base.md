...
>Chandler Carruth 是Google在c++委员会的代表，他在委员会里的地位仅次于C++之父 Bjarne Stroustrup 和来自微软的 ISO C++ 委员会主席 [Herb Sutter](https://herbsutter.com)，他也弄了个旨在简化和让c++更简单的东东[Cpp2 and cppfront](https://github.com/hsutter/cppfront)
...
- **类函数**：类的函数没有`[me: Self]` 或 `[addr me: Self*]`，则表示是一个和对象无关的函数，也叫**类函数**, 等价于C++的static成员函数。**carbon没有构造函数constructor**，习惯用`Make`命名的 **类方法**来创建实例。
    ```carbon
    class NewsAriticle {
      // 类函数
