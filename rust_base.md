# 所有权、生命周期、借用
**谈谈内存** 并非所有的内存都是平等的。在大多数编程环境下，你的程序可以访问栈(stack)、堆(heap)、寄存器(register)、文本段(text segment)、内存映射的寄存器(memory-mapped register)、内存映射文件(memory-mapped file)，有时也会有非易失性可读写存储器(Nonvolatile RAM)
