...
**`^`转移操作符** transfer operator： 是一个后缀操作符， 用在变量名称后面。**`^`** 的作用是**先结束**变量的生命周期，**再触发**调用`破坏 移动构造函数__moveinit__()`或者`窃取 移动构造函数__takeinit__()`把变量的所有权转移给接收者。
...
3. **`owned`**： 形参标记了`owned`，并**不能保证`形参`获得的是`对实参的可变引用`**，只保证 **形参一定会拥有和`实参一样的值`**，而且是这个值的唯一所有者。 形参有2种获得实参的值的方式: 通过`^`转移所有权获得、或者通过深度复制获得。
   - **实参使用了后缀`^`** ，实参就会**先结束**自己的生命周期，然后再把值的所有权转给形参。此时，**实参不能再被访问**了。 **实参是`通过转让所有权`，`不是拷贝`，让形参完全拥有了原来的的数据**，而且形参现在是这个数据的唯一所有者，对该值有唯一的可变访问权限。这个过程没有数据复制，也不会产生堆内存的申请和分配。
     - 如果实参的类型实现了`破坏 移动构造函数__moveinit__()`，那就优先调用这个方法来执行转移所有权的具体操作。
     - 如果实参的类型**没有实现**`破坏 移动构造函数__moveinit__()`，而是实现了`窃取 移动构造函数__takeinit__()`，那就调用`__takeinit__()`这个方法来执行转移所有权的具体操作。
...
Mojo强调**值语义**，不过复制有时会对性能造成重大影响。 所以Mojo提供 **`移动构造函数__moveinit__()`**，它**直接转移值的所有权**，没有发生重新分配新的堆空间、也没拷贝数据，所以效率很高。 

转移后缀操作`^`让变量的生命周期结束时，值的所有权转移后，同时会结束原来变量的生命周期（变量变为无效），以避免双重释放和释放后使用错误。它也称为**破坏性移动**，Rust的风格的移动也是这种方式。
```mojo
fn __moveinit__(inout self, owned existing: Self): 
```
