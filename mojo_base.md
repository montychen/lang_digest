...
**`^`转移操作符** transfer operator： 是一个后缀操作符， 用在变量名称后面。**`^`** 的作用是把变量的所有权转移给接收者，**接收者要注明是`owned`**(这一点很重要)。有2种情况：
   ...
   3. **`__moveinit__()`** 和 **`__takeinit__()`** stealing move基本一样，实现它们的代码都是**直接转移值的所有权，没有申请分配新的堆空间**、也没拷贝数据。 唯一的不同就是`__takeinit__()`不会结束原来变量的生命周期，原来的变量依然有效。
...
唯一的不同就是, 如果实参的类型没有实现破坏 移动构造函数`__moveinit__()`， **`^`**才会触发调用`__takeinit__()`，而且**不会结束原来变量的生命周期**，原来的变量依然有效。所以叫**窃取移动**， C++风格的移动也是这种方式。
