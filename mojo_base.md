...
<br/>
3. **`owned`**： 形参虽然标记了`owned`，并**不能保证`形参`是`对实参的可变引用`**，只保证 **形参一定会拥有和`实参一样的值`**，而且是这个值的拥有者。 形参有2种获得实参的值的方式，具体是哪一种，要看在实际传参的时候，**实参后面有没有 transfer所有权转让操作符` ^`** 
   - 函数调用传参的时候，在**实参后面使用后缀`^`** ，实参就会把所有权转给形参，并结束实参自己的生存期，此时，**实参不能再被访问**了。 **实参是`通过转让所有权`，`不是拷贝`，让形参完全拥有了原来的的数据**，而且形参现在是这个数据的唯一拥有者，作为拥有者，当然具备修改的权限。这个过程没有数据复制，也不会产生堆内存的申请和分配。

   - 函数调用传参的时候，**实参后面没有用后缀` ^`**  ，在这种情况下，**实参没有转让所有权**，实参仍然拥有原来的值，只是**实参的值会被强制复制一份给形参**，这是`值语义`。复制后的值和原来的值是彼此独立的。 实参继续是原来值的拥有者，仍然有效、可以访问；而形参是复制后的值的拥有者。 这个过程会涉及复制、堆内存的申请和分配。
4. `fn` 和 `def` 参数的默认形式
