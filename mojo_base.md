...
- **默认情况下， fn的参数是`不可变引用 borrowed `**。这是一个内存优化，以避免不必要的副本拷贝；因为反正是不可变，如果还按照默认参数的`值语义`传递，效率差。
- `fn`函数体内的变量， 通过`let` 或者 `var`来标明变量是否可变

### `def`
`def`定义的函数具有**Python style的动态性**：无论参数、函数返回值还是函数体内的变量，都**可以不用指定类型**。 所以如果在函数内用`形参`执行了某些操作，而实际传递的`实参`不支持这些操作， def函数会报错。
- **默认情况下，def的参数默认是` owned `**
...
- **为了和Python兼容， `object`是mojo提供的唯一一个`引用语义`的类型(而且使用的是Python参数传递的引用语义，简称：`Python参数引用语义`)，其它类型都是`值语义`**。因此，这两类值在一起使用时要小心，毕竟`引用语义`和`值语义`在复制时（赋值or传参）它们的行为不一致。 
  - **Python参数引用语义**：形参整个重新赋值不影响实参，局部修改影响实参，[下面有专门的描述](#python参数引用语义形参整个重新赋值不影响实参局部修改影响实参)
- 如果def定义参数时，明确声明了类型，那就不是`object对象`了，而是明确的那个类型，所以是`值语义`。
...
1. **`owned`**： 形参虽然标记了`owned`，并**不能保证`形参`是`对实参的可变引用`**，只保证 **形参一定会拥有和`实参一样的值`**，而且是这个值的唯一所有者，所有者对值具有`唯一的可变访问权限`。 形参有2种获得实参的值的方式，具体是哪一种，要看在实际传参的时候，**实参后面有没有 transfer所有权转让操作符` ^`** 
...
2. `fn` 和 `def` 参数的默认形式
   - **`fn`的参数默认是`不可变引用 borrowed`** 
   - **`def`的参数默认是`owned`**，只要参数不明确声明类型就是`object`的`引用语义`，明确声明了类型就是`值语义`。 
