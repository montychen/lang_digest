...
```
- 类似于其他静态语言，**Mojo对象的可不可变不在于类型，而在于。。。
...
```mojo
def f(position_argument, /, position_or_keyword, *, keyword_argument):
    pass
```

**缺省参数**：有默认值的参数， 而且要放在非缺省参数后面。

- 在 `/ `之**前**定义的参数, 强制为 位置参数。
- 在 `*` **后面**定义的参数, 强制为 关键字参数。传参时必需带参数名 `参数名=值`
  
**位置参数 positional**：传参时不指定参数的名称，按照参数声明的顺序`直接提供值`。

**关键字参数 keyword**：通过参数名传递参数值 `参数名=值`，这样可以不按照参数的定义顺序传递参数值，关键字参数要放在位置参数后面。

- **`*args`** 数量可变位置参数：表示接受`任意数量的位置参数`(不传参也可以)。参数前会有一个星号`*`，参数名称不一定要用`args` 这个名字，但通常用它来表示可变位置参数。所有传递的位置参数被收集到 **`args元组`** 中。传参时不能指定参数的名称。
- **`**kwargs`** 数量可变关键字参数: 要**放在最后**，用来表示函数接受`任意数量的关键字参数`(不传参也可以)。参数前会有两个星号`**`，参数名称不是一定要用`kwargs`， 所有传递的关键字参数被收集到 **`kwargs字典`** 中。
- 在定义函数的参数，`*args` 和 `**kwargs` 可以同时存在，用来接受不定数量的位置参数和关键字参数。

```mojo
fn print_nicely(**kwargs: Int) raises:  # **kwargs 收集到的值放在 字典 里
    for key in kwargs.keys():
        print(key[], "=", kwargs[key[]])  # key[] 是解引用

fn main() raises:
    print_nicely(a=7, y=8)  # `a = 7`   `y = 8`
```
...
```
#### 隐式符合trait：结构struct实现了trait要求的方法
如果一个结构实现了trait要求的所有方法，它就隐式地符合这个trait。例如，任何实现了 `__str__()` 方法的结构都隐式地符合 `Stringable` ，就可以与 `str()` 内置函数一起使用。
```mojo
@value
struct Foo:         # 实现了 __str__() 方法， 所以Foo隐私符合 Stringable 这个 trait
    fn __str__(self) -> String:
        return "foo!"
...
def main():
    ...
    list.append(MyObject(True))
    list.append(MyObject(9))
    list.append(MyObject(String("hello world")))
...
# 集合类型 `Dict` `Set` `List | InlinedFixedVector` `Optional`
### InlinedFixedVector 和 DynamicVector 
- `InlinedFixedVector[AnyRegType、Int](capacity: Int)` 初始化时用`编译时参数`指定一个**较小的初始化存储空间**，和用运行时参数指定**一个最大的存储空间**；最大的存储空间指定后，大小就固定、运行时不会也不能动态调整。
- `List[CollectionElement]` 运行时可以动态调整存储空间。 `var numbers = List[Int](1, 2, 3)`
