...
**`^`转移操作符** transfer operator： 是一个后缀操作符， 用在变量名称后面。**`^`** 的作用是把变量的所有权转移给接收者，**接收者要注明是`owned`**(这一点很重要)。有2种情况：
   ...
   3. **`__moveinit__()`** 和 **`__takeinit__()`** stealing move基本一样，实现它们的代码都是**直接转移值的所有权，没有申请分配新的堆空间**、也没拷贝数据。 唯一的不同就是`__takeinit__()`不会结束原来变量的生命周期，虽然**原来的变量依然有效**，但在`__takeinit__()`的实现中，一定要 **`人为手动`把它的值会变成`null`**。
...
`let b = a ^`  转移操作符 **`^`** 触发调用`__moveinit__`, a生命周期结束。

`let n = m`  没有使用 **`^`**, 这是一条赋值的拷贝语句，触发`__copyinit__`, m 继续有效 。

- [x] `let n = m` 没有触发`__copyinit__`执行， [已经提交bug，待验证。。。](https://github.com/modularml/mojo/issues/1529) 
...
#### 窃取 移动构造函数`__takeinit__()`: 转移所有权、原变量依然有效   C++风格
窃取移动构造函数`__takeinit__()` stealing move和 破坏移动构造函数`__moveinit__()`基本一样，都是**直接转移值的所有权**，没有发生重新分配新的堆空间、也没拷贝数据。 

唯一的不同就是, 如果实参的类型没有实现破坏 移动构造函数`__moveinit__()`， **`^`** 才会触发调用`__takeinit__()`，而且 **不会结束原来变量的生命周期**，虽然**原来的变量依然有效**，但在`__takeinit__()`的实现中，一定要 **`人为手动`把它的值会变成`null`**，避免双重释放和释放后使用，**同时保证它的析构函数仍可正常运行**。所以叫**窃取移动**， C++风格的移动也是这种方式。
```mojo
...
struct HeapArray:
    ...
    fn __takeinit__(inout self, inout existing: Self):
        ...
        # existing的生命周期没有结束，所以要人为手动把它的值会变成null，避免双重释放和释放后使用，同时保证它的析构函数仍可正常运行
        existing.size = 0
...
而且，如果一个类型是允许被复制的，通常没有理由不允许移动；反过来说就不一定合适，假如一个类型的复制成本很高，那么可以只提供移动构造函数来支持移动，但不提供`__copyinit__()`来禁止昂贵的复制操作。所以：
...
### `@register_passable` 小数据类型直接在`机器寄存器`中`以值的方式`传递

可以在结构体上添加 `@register_passable` 装饰器来告诉Mojo，该类型的值要在`机器寄存器`machine registers中传递，而且是**以值的方式**传递，而**不能通过引用传递**。

像 `Int`、`Float` 、`Bool`和 `SIMD` 等这样的 tiny data types **`小数据类型`直接在`机器寄存器`中`以值的方式`传递**，这比使用内存高效多了。

和C++和Rust等语言相比，这是一个显着的性能增强。 
