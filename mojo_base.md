...
#### `def` 和 `object`没有注明类型的对象 & `对象引用`语义
**关键字`object`** **用来表示一个`没有注明类型的对象`**。比如`def`定义函数，如果它的参数没有声明类型，或者`def`定义的函数体内的变量没有声明类型，那它们的类型就是这个`object`对象， 任何类型的值都可以传给它，`object`允许动态类型化，可以表示Mojo标准库中的任何类型， 所以会用这个值自动构建出一个`object`对象， 实际的类型会在运行时推断。


为了与Python兼容， **`object` 值使用`对象引用`语义传递**。因此， object 类型与强制值语义的参数约定不兼容。因此，如果将 object 值与其他强类型值一起使用，请小心-它们的行为可能不一致，因为 **`object` 是标准库中唯一不符合`完整值语义`的类型**。
