[Mojo](https://github.com/modularml/mojo)是第一个专门为MLIR设计的语言，它不仅要成为**Python的超集**，而且是一门**追求极致性能的系统编程语言（Focused on performance & system programming)**，以期消除两个语言的问题（Python为了提高性能，很多性能敏感的库只能使用C或C++来编写）；Mojo直接编译为原生机器代码，没有用C作为中间代码。

- Python是 **`动态类型`的`解释型语言`**。内存管理机制是：**引用计数 + 垃圾回收GC**。
- **Mojo**是 **`静态`、`内存安全memory safety`的`编译型语言`**。 它没有使用引用计数和垃圾收集器来管理内存。而是像Rust一样，通过**定义`对象的生命周期`，使用`借用检查器`来`自动管理内存`**。
...
- Swift拥抱了Objective-C运行时和语言生态系统， 通过Swift，Chris Lattner 具备保持“运行时兼容”以及如何与遗留运行时合作的丰富经验。
 ...
 Python官方的解释器是用C语言实现的，所以也叫 [CPython](https://github.com/python/cpython)，它是最广泛使用的Python解释器。期待有这么一天：CPython团队最终用Mojo语言而不是C语言实现Python的解释器 🔥 。

> MLIR多级中间表示（Multi-Level Intermediate Representation ）它在不增加复杂性的情况下扩展到各种硬件类型。 
...
### `alias` 定义`编译时常量`
与 `var` 和 `let` 一样，`alias`定义的别名也是**有作用域**的，比如，可以在函数中定义仅在函数内使用的本地别名。
```mojo
...
### stack栈 & `编译时已知` 而且 `存储空间固定` 的变量
一般来说，所有编程语言都以相同的方式使用`call stack调用栈`：当调用函数时，编译器在栈上stack分配一个内存块，该内存块的大小**正好**可以存储`执行逻辑` 和所有 **`编译时已知`** 而且 **`存储空间固定`** 的变量。当另一个函数被调用时，它的数据同样被添加到栈的顶部。当一个函数完成时，它在栈中的所有数据都将被销毁，以便内存可用于其他代码。


### heap堆 & 编译时`存储空间未知` 和 运行时`存储空间会变`的变量
任何在 **`编译时存储空间未知`** 和 **运行时会`动态改变存储空间`** 的变量都存储在堆heap中，堆是一个更大的内存区域，允许在运行时进行动态内存访问。 堆上的内存分配和管理比栈更为复杂和耗时，因为要先找到足够空间的存储位置、并进行内存管理。
...
- **Parametric编译时参数** 要求放在**方夸号里`[ ]`**，它的值要求是**编译时是已知的、是编译时常量，所以不存在内存管理**。 后续所说参数的**所有权、以及内存管理都是针对`Argument运行时参数`**
  - `Parametric编译时参数 []`是编译时常量，不存在内存管理的问题。

- **Argument 运行时参数** 要求放在**括号里`( )`**，它的值要求是在运行时是已知的，所以也叫**运行时参数**。

### Parametric 编译时参数 Fully-bound、Partially-bound、unbound
- `完全绑定Fully-bound`：指定了所有`编译时参数`
- `部分绑定Partially bound`：只指定了部分编译时参数，其中未绑定参数可在以后提供。
- `没绑定unbound`

部分绑定的例子：用`alias`定义一个类型别名： Scalar
```mojo
...
### 可以访问`类型的编译时参数` 
Mojo的许多类型是需要指定编译时参数的，比如`SIMD[type: DType, size: Int]` 或者 `Pointer[type: AnyRegType, address_space: AddressSpace]`等等。 重要的是：**无论是在`函数签名`还是在`函数内`都可以访问`类型的编译时参数`**。 

例子：确保2个输入的SIMD参数长度一致。

```mojo
# 在函数签名里 访问SIMD的编译时参数type 和 size。这里用v1.size确保2个SIMD的长度一致
fn interleave(v1: SIMD, v2: SIMD[v1.type, v1.size]) -> SIMD[v1.type, v1.size * 2]:
    var result = SIMD[v1.type, v1.size * 2]()  # 在函数内通过v1 访问SIMD的编译时参数type 和 size
...
fn main():
    ...
    let b = SIMD[DType.int16, 4](10, 20, 30, 40)
    let c = interleave(a, b)
    for i in range(c.size):
        print_no_newline(c[i], " ")

```
### 类型的编译时参数 `没绑定` 或 `部分绑定`都可用作函数的`运行时参数类型`
 `类型的编译时参数`即使完全`没绑定unbound`，或者只是`部分绑定`(没绑定的部分，可以用下划线 `_` 明确表示出来，不用也行)，都可以直接用作函数**运行时参数的类型**，等到函数实际调用，再传递完全绑定的实参。  

#### 例子： 完全`没绑定unbound` 用作 函数`运行时参数的类型`

```mojo
# fn print_params[t: DType, s: Int](vec: SIMD[t, s]):  和下面这句功能一样，都是对的
fn print_params(vec: SIMD):   # SIMD的编译时参数完全没有绑定， 直接用作运行时参数 vec 的类型。

    print(vec.type)           # 通过vec访问SIMD的编译时参数type
    print(vec.size)
...
#### 例子： `部分绑定的` 用作 函数`运行时参数的类型`。
没绑定的部分，也可以直接用下划线 `_` 明确表示出来，不用也行。
```mojo
@value
...
fn eat1(f: Fudge[5]):                   # 部分绑定
    # fn eat[c: Int](f: Fudge[5, c]):   # 等同上面的语句，都OK
    print("Ate... " + str(f))
    print("sugar=", f.sugar, " cream=", f.cream)    # 用f访问类型的编译时参数

fn eat2(f: Fudge[25, _]):               # 没绑定的部分，用下划线 _ 明确表示出来。
    print("\nAte=== " + str(f))
    print("sugar=", f.sugar, " cream=", f.cream)    # 用f访问类型的编译时参数
...
fn main():
    eat1(Fudge[5, 8]())
    # eat(Fudge[6, 5]())  # 报错，类型不匹配。 在定义eat1函数时，已经明确写明Fudge的第一个参数是 5
...
- **唯一可以定义`__init__`这个生命周期方法**，不定义也可以。不过这里定义的`__init__`有点不同，它的参数不用`inout self`，它是**静态**的、`新创建的实例`作为`函数的结果值`返回。
...
#### `capturing` 闭包捕获了外部的值
如果闭包捕获或者说使用了outer-scope外部的值，那么高阶函数的参数要用 `capturing`声明。 闭包没有捕获外部值，高阶函数用`capturing`也不会报错。
```mojo
...
`捕获了外部值的闭包`要用作`编译时参数`Parametric， 闭包定义的时候，要在前面加`@parameter`，不然要报错。
```mojo
...
fn create_closure():
    ...
    @parameter              # 捕获了外部值的闭包，要用作编译时参数，
    fn add(i: Int) -> Int:
        return x + i       # 捕获了外部值 x

    let y = use_closure[add](2)   # 闭包用作 编译时参数
    print(y)
...
# 协程 coroutine
面对高并发，采取多线程的话，一方面线程间的切换成本较高，另一方面是系统对线程的数量有限制，因此多线程的结构没有优势，因此协程技术应运而生。
所谓 **`协程`，即`用户级线程`**，是一种将异步代码同步化的编程机制，使得程序的执行流可以在多个并行事务之间切换但又不必承担切换带来的过高的性能损耗。协程分为两大类，一类是stackful**有栈协程**，代表是go语言的goroutine。另一类是stackless**无栈协程**，通常使用`async`、`await`语法，代表是Rust。

`有栈协程`上下文切换靠**切换整个系统栈**，而 **`无栈协程`共享同一个系统栈**，所以切换时协程必须主动返回到开始调用协程的地方，下次需要自己恢复当时的现场。一般**无栈协程**会通过一个用户堆栈或者类似的数据结构来完成`await`调用的保存和恢复
- 有栈协程优点：执行效率比无栈协程差；代码**没有颜色**问题，更容易使用、更易理解。
- 无栈协程优缺点：执行效率高；代码**有颜色**问题，代码比稍微难懂一些；
