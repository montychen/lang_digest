...
- **`*args`** 数量可变位置参数：表示接受`任意数量的位置参数`(不传参也可以)。参数前会有一个星号`*`，参数名称不一定要用`args` 这个名字，但通常用它来表示可变位置参数。所有传递的位置参数被收集到一个`元组`中。传参时不能指定参数的名称。
- **`**kwargs`** 数量可变关键字参数: 要**放在最后**，用来表示函数接受`任意数量的关键字参数`(不传参也可以)。参数前会有两个星号`**`，参数名称不是一定要用`kwargs`， 所有传递的关键字参数被收集到一个`字典`中。
