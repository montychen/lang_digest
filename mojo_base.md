...
通过跟踪每个值的`生存期`并在正确的时间销毁每个值（并且只销毁一次）。对于每一个值，Mojo不强制`独占访问`。只是强调：
 1. 每个值 **`同一时间只有一个所有者`，所有者在`生命周期结束时， Mojo会销毁该值`**。
<br/>

2. 每个值只能有`一个可变引用inout`， 可以有`多个不可变引用 borrowed`；
<br>
...
`inout 可变引用` ：形参接收到的是一个`可变引用mutable reference`，这意味着**可以读取和改变值**。

`owned`：形参获取了实参的`所有权`。这意味着形参具有**独占的可变访问**权限， 实参不再具有对该值的访问权限。通常调用者应该将实参的所有权转给这个函数，但这并不一定会发生，这可能是一个副本。
