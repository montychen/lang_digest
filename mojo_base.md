...
### 参数的 `borrowed` `inout` `owned` 和 `^`转移操作符 

**`^`转移操作符** transfer operator： 是一个后缀操作符， 用在变量名称后面。**`^`** 的作用是**先结束**变量的生命周期，**再触发**调用`破坏 移动构造函数__moveinit__()`
...
3. **`owned`**： 形参虽然标记了`owned`，并**不能保证`形参`是`对实参的可变引用`**，只保证 **形参一定会拥有和`实参一样的值`**，而且是这个值的唯一所有者，所有者对值具有`唯一的可变访问权限`。 形参有2种获得实参的值的方式，具体是哪一种，要看在实际传参的时候，**实参后面有没有 transfer所有权转让操作符` ^`** 
   - **实参后面用了`^`** ，实参就会**先结束**自己的生命周期，然后再把值的所有权转给形参。此时，**实参不能再被访问**了。 **实参是`通过转让所有权`，`不是拷贝`，让形参完全拥有了原来的的数据**，而且形参现在是这个数据的唯一所有者，对该值有唯一的可变访问权限。这个过程没有数据复制，也不会产生堆内存的申请和分配。
    - **实参后面没有用` ^`**，实参没有结束自己的生命周期，**实参也没有转让所有权**，只是**实参的值会复制一份给形参**，这是 **`值语义`**，就是单纯的复制，要求执行的是深度复制。复制后的值和原来的值彼此独立的。 实参继续是原来值的唯一所有者，仍然有效、可以访问；而形参是复制后的值的唯一所有者。 这个过程会涉及复制、堆内存的申请和分配。
       - 当值类型用作`owned参数`，为了确保实参在没有使用转移操作符`^` 的情况下也能正常运行，值类型必须确保是深度可复制的（通过实现 `__copyinit__()`来做到）。 
...
如果一个类型实现了`破坏 移动构造函数__moveinit__()`，并且该类型的值是作为实参，传给owned参数，同时实参的生命周期在同一点结束（使用或不使用 `^` 传输操作符），Mojo**可能**会调用此方法。

如果一个类型**没有实现**`破坏 移动构造函数__moveinit__()`，而是实现了`窃取 移动构造函数__takeinit__()`，并且该类型的值是作为实参，传给owned参数，同时实参的生命周期在同一点结束（使用或不使用 `^` 传输操作符），Mojo**可能**会调用此方法。

如果一个类型既没有实现`破坏 移动构造函数__moveinit__()`也没有实现 `窃取 移动构造函数 __takeinit__()` ，Mojo可以通过简单的把`caller’s stack调用者堆栈中值的引用`转移给对方，来实现所有权转移。
...
如果不想自己的类型被复制，就不要实现`__copyinit__()`。只有实现了 `复制构造函数__copyinit__()`，它的实例才支持复制，才可以用在赋值操作`=`里(触发调用`__copyinit__()`)。 复制其实就是使用已经存在的值，构造出另一个**全新的对象**。 所以才叫**复制构造函数**。
...
如果自己定义的类型，**没有手动使用`Pointer`** 在堆上分配内存，而且**所有`结构体的字段`使用的都是`可自销毁的数据类型`**（例如 Int 、 Bool 、 String 等），那么你可以在struct的定义中添加 **`@value` 装饰器**，让Mojo帮你生成 **`__init__()`** 、 **`__copyinit__()`** 和 **`__moveinit__()`** 。 **助记：一棵木**
- 上述的3个生命周期方法，如果有自己的实现，编译器就会使用你自己定义的。只要没有自己实现，才会使用`@value`装饰器帮你生成的。
...
### 破坏 移动构造函数`__moveinit__()`： 转移所有权、原变量失效

复制有时会对性能造成重大影响。 所以Mojo提供 **`移动构造函数__moveinit__()`**，它**直接转移值的所有权**，没有发生重新分配新的堆空间、也没拷贝数据，所以效率很高。 值的所有权转移后，同时会结束原来变量的生命周期（变量变为无效），以避免双重释放和释放后使用错误。它也称为**破坏性移动**，Rust的风格的移动也是这种方式。
```mojo
fn __moveinit__(inout self, owned existing: Self): 
```
`__moveinit__()` 的关键是**形参要标记为` owned`**，意味着将获得实参的唯一所有权。这是一个双划线的dunder方法，Mojo会确保它的调用时机，保证形参existing获得的是实参的可变引用，而不是复制的副本（即使形参声明为`owned` ，如果方法调用的时候，实参后面没有使用` ^`，则形参接收到的实参的副本, [上面有owned的说明](#参数之-borrowed-inout-owned-和-transfer所有权转让操作符)）。


### 窃取 移动构造函数`__takeinit__()`: 转移所有权、原变量依然有效
窃取移动构造函数`__takeinit__()` stealing move和 破坏移动构造函数`__moveinit__()`基本一样，都是**直接转移值的所有权**，没有发生重新分配新的堆空间、也没拷贝数据。 唯一的不同就是`__takeinit__()`**不会结束原来变量的生命周期**（原来的变量依然有效），所以叫**窃取移动**， C++风格的移动也是这种方式。
