...
### Mojo的 Ownership 所有权规则 
Mojo编译器实现了一个借用检查器，可以跟踪每个值的`生存期`并在正确的时间销毁每个值（并且只销毁一次）。对于每一个值，Mojo不强制`独占访问`。只是强调：
...
### Mojo 参数约定：`borrowed` `inout` `owned`
**`borrowed 不可变引用`**: 形参接收到的是一个`不可变的引用  immutable reference`，这意味着**可以读取值，但不能修改**。

**`inout 可变引用`** ：形参接收到的是一个`可变引用mutable reference`，这意味着**可以读取和改变值**。 在函数内**对形参的修改，是会影响外面的实参**的。


它只保证 **形参一定会拥有`实参的值`，的唯一拥有者，而且具备可变访问权限** 。 因为在于传参的时候，实参后面可能没有使用 **transfer所有权转让操作符  ` ^`**

**`owned`**：形参获取了实参值的`所有权`。这意味着形参具有**独占的可变访问**权限， 实参不再具有对该值的访问权限。通常调用者应该将实参的所有权转给这个函数，但这并不一定会发生，这可能是一个副本。
...
### `owned` &  transfer所有权转让操作符  ` ^`

形参虽然标记了`owned`，并**不能保证`形参`是`对实参的可变引用`**，它只保证 **形参是`从实参拷贝过来的值`的唯一拥有者，而且具备可变访问权限** 。 因为在于传参的时候，实参后面可能没有使用 **transfer所有权转让操作符  ` ^`**

- 函数调用，在传参的时候，通常在 **`实参`后面使用后缀` ^ `** ，**`实参`就会把所有权转给`形参`，并结束实参自己的生存期**，此时，**实参不能再被访问**了。 实参是通过转让所有权，让形参完全拥有了原来的的数据，这里**不会产生数据复制**，也不会产生堆内存的申请和分配。

- 函数调用，在传参的时候，**`实参`后面`没有用后缀 ^ `** ，在这种情况下，**实参的值被强制`复制`到形参**， 复制后，**形参是`从实参拷贝过来的值`的唯一拥有者**，而且具备可变访问权限。同时实参仍然是有效。 这个过程会涉及复制、堆内存的申请和分配。
