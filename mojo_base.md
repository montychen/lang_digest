...
Mojo使用了第三种称为**所有权ownership**的方法，它依赖于**程序员在传递值时必须遵循一些规则**。Ownership 所有权的核心规则: **对于一个值，Mojo不强制`独占访问`，允许同一个值被多个变量`同时读取`，只是强调 `每个值同一时间只有一个所有者`，所有者在`生命周期结束时， Mojo会销毁该值`**。
...
通过跟踪每个值的`生存期`并在正确的时间销毁每个值（并且只销毁一次）。对于一个值，Mojo不强制`独占访问`，允许同一个值被多个变量`同时读取`，只是强调 **`每个值同一时间只有一个所有者`，所有者在`生命周期结束时， Mojo会销毁该值`**。

### Mojo 参数约定：`borrowed` `inout` `owned`
`borrowed`: 形参接收到的是一个`不可变的引用  immutable reference`，这意味着**可以读取值，但不能修改**。

`inout` ：形参接收到的是一个`可变引用mutable reference`，这意味着**可以读取和改变值**。

`owned`：形参获取了实参的`所有权`。这意味着形参具有**独占的可变访问**权限， 实参不再具有对该值的访问权限。通常调用者应该将所有权转移给这个函数，但这并不总是发生，这可能是一个副本。
