...
Mojo不强制使用**值语义value semantics**或**引用语义reference semantics**。它支持这两种类型，并允许每种类型自己定义如何`创建create`、`复制copy`和`移动move`。



### 值语义value semantics

**值语义value semantics**: 意味着每个变量都对一个值具有**唯一的访问权限**，并且该变量范围之外的任何代码都不能修改其值。
...
#### 函数参数
**Mojo默认为`运行时参数argument设置了值语义`的参数行为**。 唯一特殊的是**def中没注明类型的都是`object对象`**, 它是按照`对象引用语义`进行传递。

如果def定义参数时，明确声明了类型，那就不是`object对象`了，默认就还是`值语义`。
