...
**`^`转移操作符** transfer operator： 是一个后缀操作符， 用在变量名称后面。**`^`** 的作用是把变量的所有权转移给接收者，**接收者要注明是`owned`**。有2种情况：
   ...
   3. **破坏 移动构造函数`__moveinit__()`** 和 **窃取 移动构造函数`__takeinit__()`** stealing move基本一样，实现它们的代码都是**直接转移值的所有权，没有申请分配新的堆空间**、也没拷贝数据。 唯一的不同就是__takeinit__()不会结束原来变量的生命周期，原来的变量依然有效。
...
通过转移操作符`^`先触发变量结束自己的生命周期，然后再执行`__moveinit__()`把值的所有权转移给接收者。，同时会结束原来变量的生命周期（变量变为无效），以避免双重释放和释放后使用错误。它也称为**破坏性移动**，Rust的风格的移动也是这种方式。
```mojo
fn __moveinit__(inout self, owned existing: Self): 
```
