Mojo是第一个专门为MLIR设计的语言，它不仅要成为**Python的超集**，而且是一门**追求极致性能的系统编程语言（Focused on performance & system programming)**，以期消除两个语言的问题（Python为了提高性能，很多性能敏感的库只能使用C或C++来编写）；Mojo直接编译为原生机器代码，没有用C作为中间代码。
...
- Swift拥抱了Objective-C运行时和语言生态系统， 通过Swift，Chris Lattner 具备保持“运行时兼容”以及如何与遗留运行时合作的丰富经验。
 ...
 Python官方的解释器实现用C语言实现的，所以也叫**CPython**，它是最广泛使用的Python解释器。期待有这么一天：CPython团队最终用Mojo语言而不是C语言实现Python的解释器 🔥 。
...
### 可以访问`Fully-bound类型`的属性
在`函数内`以及`函数签名`的地方，都可以访问`Fully-bound类型`的属性。 

例子：确保2个输入的SIMD长度一致。

```mojo
# 在函数参数、返回值类型里 访问`Fully-bound类型`的属性：这里是确保2个SIMD的长度一致
fn interleave(v1: SIMD, v2: SIMD[v1.type, v1.size]) -> SIMD[v1.type, v1.size * 2]:
    var result = SIMD[v1.type, v1.size * 2]()  # 在函数内访问`Fully-bound类型`的属性 
...
### `编译时参数`的类型 可以是`没绑定unbound`
完全`没绑定unbound`的类型, 可以用作`编译时参数`的类型，到函数调用时再传递完全绑定的实参。  

```mojo
# fn print_params[t: DType, s: Int](vec: SIMD[t, s]):  和下面这句功能一样，都是对的
fn print_params(vec: SIMD):   # 这里的SIMD 是完全没有绑定的类型 unbound的

    print(vec.type)  # 形参vec访问`Fully-bound类型`的属性。 因为实际调用的时候，实参是`Fully-bound`的。
    print(vec.size)
