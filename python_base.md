...
不可变容器对象(比如元组），在函数内**给整个元组重新赋值，函数外的`实参`就不会变**。
```python
...
不可变容器对象，包含的元素是可变对象，在函数中对包含的可变对象元素进行修改（局部修改）, 虽然不可变对象所指向的内存地址并没有改变，**对象还是原来的对象**， 但**函数外的实参会受影响**，毕竟不可变容器对象，所包含的可变对象已经被修改了。
