...
### 可变对象：`list` `dict` `set`。 
  修改可变对象所包含元素的值，并不会改变**可变对象所指向的内存地址**，也就是说**对象还是原来的对象**。 只有给整个可变对象重新赋值，才会指向另一个内存地址。
```python
...
### 不可变对象：`tuple` `string` `int` `float` `bool`
 不可变对象的值不可改变，要想给它不同的值，只能重新赋一个新的对象。
```python
x = 3
...
### 不可变对象是容器类型时，比如`元组`
正常的元组我们是不可以修改它元素的值，但是，如果**元组包含的元素是一个可变对象**，比如列表， 那么修改这个列表的值是可以的，而且元组所指向的内存地址并没有改变，还是原来的对象。 只有给整个元组重新赋值，它的地址才会改变。
```python
a = ([1], 100)     # 定义一个不可变对象，这里是元组a
print(a, id(a))    # ([1], 100) 4306542976
a[0].append(2)     # a[0]是一个列表， 列表是可变对象
a[0].append(3)
print(a, id(a))    # ([1, 2, 3], 100) 4306542976  a的地址没变，还是原来的对象
```
