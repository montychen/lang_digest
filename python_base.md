...
# `r"..."` 或者 `R"..."` 原始字符串
 带有`r` 或者 `R`前缀的字符串是**原始字符串标**，该字符串中的**特殊符号不会被转义**。 因此 `r"\n"` 表示包含 `\` 和 `n` 两个字符的字符串，而 `"\n"` 则表示只包含一个换行符的字符串。
```python
s = '你好\t世界'
print(s)               # 你好    世界

rs = r'你好\t世界'       # 这是一个原始字符串， 不转义
print(rs)              # 你好\t世界
```
...
## ` * `列表或元组前一个星号 和 ` ** `字典前两个星号
` * `列表或元组前一个星号，是将列表中的元素**解包成单独的元素**，比如，这样可以方便作为参数传给函数。
```python
def add(a, b):
    return a + b
