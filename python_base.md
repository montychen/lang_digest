...
- 以**双下划线开头**且以**至多一个下划线结尾**的成员变量或方法，会进行名称转写。


### 为什么需要名称转写
**名称转写(name mangling)**：以双下划线开头，并以**最多**<u>一个下划线结尾</u>的标识符，例如__X，会被转写为_classname__X，其中classname为类名。这个机制实现起来非常简单，而且很大程度避免了调用者的误访问，但并不能像Java的private限定符那样完全杜绝外部的访问。
```python
 class A():
     def __init__(self):
...
# 一切皆对象
Python 中一切皆对象，是的，数字、列表、字典等等啥都是对象。除此之外，函数、类本身也是对象。变量充其量就是对象的一个引用，变量赋值操作 `=` 其实是**把一个名字绑定到对象**上name binding，通俗语气讲，变量就是一个标签，一个名字，仅此而已，不像 C 语言那样。
...
根据函数参数类型的不同，函数实参可以通过**值传递** 或 **引用传递**
- 如果实参的数据类型是可变对象（列表、字典），则函数参数将通过引用传递。
```python
...
print("data的值已经改变：", data)   # [1, 2, 3, 4]
```

- 将不可变对象immutable，比如 **字符串，元组，int/float** 作为参数传入函数中，在函数中修改其值，不会修改该变量真正的值。如果想要更改，可以用global来申明一下。
- 但如果将可变对象mutable：**字典，列表**作为参数传入函数中，在函数中修改值，会改变原变量的值。


# 可变 & 不可变对象
Python中的函数通过引用传参。

- 不可变对象immutable： **数字、字符串、元组**
- 可变对象mutable：**列表、字典、集合**

### 不可变对象
对于不可变对象，我们无法改变对象自身，但是可以改变变量所指向的对象。 *****
这句也不全对， 例如对应不可变的容器类型，比如”元组“，可以修改它元素的值，元组所指向的地址并没改变。 只有给整个元组重新赋值，它的地址才会改变。
```python
x = 3
...
```
我们先是让变量 x 指向了整数 3，然后又让 x 指向了整数 4。我们无法改变整数 3 自身，但是可以改变 x 所指向的对象。
...
def test_immutable_for_simpledata(args):
    print('============')
    print(args, id(args))
    args = args + 10       # 给args 重新赋值，其实是创建新的对象，所以args的地址变了
    print(args, id(args))
    print('============')
...
def test_immutable_for_tuple(args):
    print('============')
    print(cur, id(args))
    args[0].append(5)  # 形参args修改了不可变元组里包含的可变的元素列表[0]， 改为[0,5]， 会影响函数外的实参cur
    print(cur, id(args))
    print('============')
...
不可变形参tuple, 在函数内部修改它元素的值，函数外的实参也跟着变了;  如果是给整个元组重新赋值，函数外的实参就不会变。
```python
def change_tuple_elem(atuple):
    print("    ", atuple, id(atuple))
    atuple[0] = [100, 200]
    print("    ", atuple, id(atuple))


def re_assign_tuple(atuple):
    print("    ", atuple, id(atuple))
    atuple = [1000, 2000, 3000]
    print("    ", atuple, id(atuple))
...
- 可变对象：`list` `dict` `set`。 可以直接修改可变对象所包含元素的值，并不会改变**可变对象所指向的内存地址**，也就是说**对象还是原来的对象**。 只有给整个可变对象重新赋值，才会指向另一个内存地址。
- 不可变对象：`tuple` `string` `int` `float` `bool`。 不可变对象的值不可改变，要想给它赋不同的值，只能给它重新赋一个新的对象。



### 可变对象作为参数
可变对象作为参数传入时，在函数中对其进行修改，是会影响到全局中的这个变量值的，因为函数是对该对象所指向的内存地址进行了修改。