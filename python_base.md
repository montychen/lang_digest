...
- 可变对象：`list` `dict` `set`。 修改可变对象所包含元素的值，并不会改变**可变对象所指向的内存地址**，也就是说**对象还是原来的对象**。 只有给整个可变对象重新赋值，才会指向另一个内存地址。
...
### 不可变对象
不可变对象的值不可改变，要想给它赋不同的值，只能给它重新赋一个新的对象。 
```python
x = 3
...
### 可变对象
修改可变对象所包含元素的值，并不会改变可变对象所指向的内存地址，也就是说对象还是原来的对象。 只有给整个可变对象重新赋值，才会指向另一个内存地址。
```python
