...
- 以`_`**单下划线开头**的成员变量或方法，是设计者不想暴露给外部的API，使用者不应该进行访问。 如果执意要访问该变量，直接通过这个名字还是可以访问的。
- 以`__`**双下划线开头**且以`_`**至多一个下划线结尾**的成员变量或方法，会进行名称转写。如果执意要访问该变量，直接通过这个名字是访问不了的，因为已经被改写成其它名称了。
