...
# 下划线 `_`
- Python保留了双下划线开始和结束的名称，用于特殊用途。比如
  - **`__init__`对象构造函数**
  - **`__del__`** 等同于类的析构函数
  - **`__call__(self, *args)`** 使对象的实例能够像函数一样被调用，同时不影响实例本身的生命周期
  - **`__iter__()`** 如果一个对象内部实现了这个方法，那么该对象就是一个可迭代对象iterable
- 单后缀下划线: 如果我们定义的名称和Python已有的关键字（如class，sum之类的）冲突, 那么可以再名称后面加一个下划线 **`_`**来解决名字冲突。 但是不建议这样命名，有那么多名称，何必去定义关键词相关的名称呢，这样也容易引起混淆。
...
# 可迭代对象(iterable)
如果一个对象内部实现了 **`__iter__()`** 方法，那么该对象就是一个iterable可迭代对象。
