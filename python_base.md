...
# 一切皆对象
Python 中一切皆对象，是的，数字、列表、字典等等啥都是对象。其次，Python 的变量充其量就是对象的一个引用，变量赋值操作 `=` 其实是把一个名字绑定到对象上，通俗语气讲，变量就是一个标签，一个名字，仅此而已，不像 C 语言那样。
...
print(id(x))
```
例子中的 `x`, `y` **这些东西叫做name**，而**不是叫变量**，因为如果使用一个未定义的东西xx，python会报  NameError: name 'xx' is not defined。


# 函数
### python参数传递 & `name binding`
