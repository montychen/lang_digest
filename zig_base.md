...
```
### packed struct 压缩结构体
**packed struct压缩结构体**：zig会保证压缩结构体的内存大小、严格按照它的声明顺序， 并且成员间不会为了字节对齐而填充；也就是成员会按照声明顺序而且彼此间 **无间隙地打包在内存中**。

- packed 的意思是在 struct 和 union 结构中不添加填充字节。
- packed一般会以节省内存、但降低运行性能为代价，因为cpu在处理边界对齐的数据时会更有效。

**普通结构体**
- 普通的结构体，Zig不保证结构体成员在内存中的顺序；也不保证内存占用大小；
- 普通结构成员间保证是字节是对齐的（也叫ABI对齐），所以CPU访问速度会更快。因为CPU种类多，比如32位和64位的CPU， 为了对齐，成员间填充数量会不同、所以保证不了大小，当然占用的内存也会多一点
...
```
> endian 字节序
>- big endian   （大端模式）：最高字节在地址最低位，最低字节在地址最高位，依次排列。
>- little endian（小端模式）：最低字节在最低位，最高字节在最高位，反序排列。
...
```
## `"hello".*` 是什么鬼！
`字符串字面量"hello"`是保存在应用程序`.rodata`里的常量指针，在整个app生命周期内只读且全局有效。  `"hello".*`是指针的解引用，执行这个语句，可以获得指针指向的内容，这些内容被复制到新分配的**栈内存**（假设该语句在一个函数内），此后您可以修改，因为该内存是您的，此时它的类型是`[5:0]u8`.
```zig
const std = @import("std");
